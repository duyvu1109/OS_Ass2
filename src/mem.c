...
int memmory_available_to_allocate(int num_pages, struct pcb_t * proc) {
	...
	int i = 0;
	int count_pages = 0; // count free pages
	while (i<NUM_PAGES)
	{
		if (_mem_stat[i].proc == 0) {
			if (++count_pages == num_pages) break;
		}
		i++;

	}
	
	if (count_pages < num_pages) return 0;
...
void allocate_memory_available(int ret_mem, int num_pages, struct pcb_t * proc) {
	...
	int count_pages = 0; // count allocated pages
	...
	for (; i < NUM_PAGES; i++) {
		...
		addr_t v_address = ret_mem + count_pages * PAGE_SIZE; // virtual address of this page
...
addr_t alloc_mem(uint32_t size, struct pcb_t * proc) {
	...
	if (mem_avail) {
		 ...
		 */
		allocate_memory_available(mem, num_pages, proc);
	}
	...
	for (i = 0; i < num_pages; i++) {
		...
		for (j = 0; j < page_table->size; j++) {
			if (page_table->table[j].v_index == v_page) {
				int tmp = --page_table->size;
				page_table->table[j] = page_table->table[tmp];
				break;
			}
		}
